    ...
    <footer className="bg-gray-50 border-t border-gray-200 mt-0">
      <div className="max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 md:grid-cols-3 gap-6 items-center text-center md:text-left">
        <div className="flex flex-col items-center md:items-start">
          <div className="flex items-center mb-2 space-x-2 justify-center md:justify-start">
            <img src={logo} alt="Postify Logo" className="w-7 h-7 object-contain" />
            <h2 className="text-xl font-bold text-blue-600 ">Postify</h2>
          </div>

          <p className="text-gray-600 text-sm leading-relaxed max-w-xs">
            A modern platform for sharing stories and building communities through creativity, connection, and
            meaningful posts.
          </p>
        </div>

        <div className="flex flex-col md:flex-row items-center justify-center gap-4 text-sm text-gray-600 font-medium">
          <Link to="/legal#about" className="hover:text-blue-600 transition-colors duration-200">
            About
          </Link>
          <Link to="/legal#privacy" className="hover:text-blue-600 transition-colors duration-200">
            Privacy Policy
          </Link>
          <Link to="/legal#terms" className="hover:text-blue-600 transition-colors duration-200">
            Terms of Service
          </Link>
        </div>
      ...
      <div className="py-4 px-6 text-center">
        <p className="text-xs text-gray-500">
          © {new Date().getFullYear()} <span className="font-semibold text-blue-600">Postify</span>. Built with 💙 and
          creativity for the writing community.
        </p>
      </div>
    </footer>
  );

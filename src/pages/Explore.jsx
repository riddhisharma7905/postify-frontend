...
const HeroCard = ({ post, onClick }) => (
  <div
    className="group bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer mb-12 flex flex-col md:flex-row"
    onClick={onClick}
  >
    <div className="relative md:w-1/2 h-80 md:h-auto overflow-hidden flex-shrink-0">
      ...
      {post.views > 50 && (
        <div className="absolute top-5 left-5 bg-orange-500 text-white text-xs font-black uppercase tracking-wide px-3 py-1.5 rounded-xl flex items-center gap-1.5 shadow-lg">
          <Flame size={12} /> Trending
        </div>
      )}
      {post.category && (
        <div className="absolute top-5 right-5 bg-blue-600 text-white text-xs font-black uppercase tracking-wide px-3 py-1.5 rounded-xl shadow-lg">
          {post.category}
        </div>
      )}
    </div>
    <div className="p-10 md:p-14 flex flex-col justify-center flex-1">
      <div className="text-xs font-black text-blue-600 uppercase tracking-wide mb-5 opacity-70">
        FEATURED STORY ·{' '}
        {new Date(post.createdAt).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}
      </div>
      <h2 className="text-3xl md:text-4xl font-black text-gray-900 mb-5 group-hover:text-blue-600 transition-colors leading-tight ">
        {post.title}
      </h2>
      <p className="text-gray-500 text-base leading-loose truncate mb-8 font-medium">{post.content}</p>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex gap-3 text-xs font-bold text-gray-400 mt-0.5">
...
const PostCard = ({ post, onClick }) => (
  <div
    className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 flex flex-col cursor-pointer"
    onClick={onClick}
  >
    <div className="relative h-56 overflow-hidden">
      <img
        ...
        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
      />
      <div className="absolute inset-0 bg-black opacity-10 opacity-0 group-hover:opacity-100 transition-opacity" />
      {post.views > 50 && (
        <div className="absolute top-4 left-4 bg-orange-500 text-white text-xs font-black uppercase tracking-wide px-2.5 py-1 rounded-lg flex items-center gap-1 shadow-md">
          <Flame size={10} /> Hot
        </div>
      )}
      {post.category && (
        <div className="absolute top-4 right-4 bg-white text-blue-600 text-xs font-black uppercase tracking-wide px-2.5 py-1 rounded-lg shadow-md">
          {post.category}
        </div>
      )}
    </div>
    ...
    <div className="p-8 flex-1 flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div className="text-xs font-black text-blue-600 uppercase tracking-wide opacity-75">
        ...
        <div className="flex gap-2">
          {post.tags?.slice(0, 2).map((tag, i) => (
            <span
              key={i}
              className="px-2 py-0.5 bg-gray-50 text-gray-400 text-xs font-bold rounded-lg group-hover:bg-blue-50 group-hover:text-blue-500 transition-colors"
            >
              #{tag}
            </span>
          ))}
      ...
      <h3 className="text-xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors mb-3 truncate leading-snug">
        {post.title}
      </h3>
      <p className="text-gray-400 text-sm truncate mb-8 flex-1 leading-relaxed font-medium">{post.content}</p>
      ...
      <div className="flex items-center justify-between pt-5 border-t border-gray-50">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-bold text-gray-700">{post.author?.name || 'Creator'}</span>
        </div>
        <div className="flex gap-3 text-sm font-bold text-gray-300">
          <span className="flex items-center gap-1">
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-0 font-inter">
      <section className="bg-white border-b border-gray-100 pt-20 pb-12 px-6">
        <div className="max-w-5xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-2xl text-sm font-black uppercase tracking-wide mb-6 border border-blue-100">
          ...
          <div className="relative max-w-2xl mx-auto group">
            ...
            <input
              ...
              className="w-full pl-14 pr-14 py-5 bg-gray-50 border border-gray-100 rounded-2xl focus:bg-white focus:ring-4 focus:ring-blue-100 focus:border-blue-400 transition-all outline-none text-gray-900 font-medium text-lg shadow-sm"
            />
            {searchQuery && (
              <button
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`px-5 py-2 rounded-2xl text-sm font-bold transition-all ${
                  activeCategory === cat
                    ? 'bg-blue-600 text-white shadow-lg shadow-md'
                    : 'bg-white text-gray-500 border border-gray-100 hover:border-blue-100 hover:text-blue-600 hover:bg-blue-50'
                }`}
              >
                {cat}
              </button>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
              {(debouncedSearch.trim() ? posts : restPosts).map((post) => (
                <PostCard key={post._id} post={post} onClick={() => navigate(`/post/${post.slug || post._id}`)} />
              ))}
            </div>
          </>
        ) : (
          <div className="bg-white p-20 rounded-3xl shadow-sm border border-gray-100 text-center flex flex-col items-center justify-center my-4">
            ...
            <h3 className="text-gray-900 text-2xl font-black mb-3">No results found</h3>
            <p className="text-gray-400 text-sm max-w-sm mx-auto mb-10 leading-relaxed font-medium">
              We couldn't find any matches for "<strong>{searchQuery || activeCategory}</strong>". Try a different
              keyword or category.
            </p>
            <button
              onClick={() => {

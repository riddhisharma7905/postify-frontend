...
const GridCard = ({ post, isPinned, isOwner, onPin, onClick }) => (
  <div
    className="relative group aspect-square cursor-pointer overflow-hidden rounded-xl bg-gray-50 border border-gray-100"
    onClick={onClick}
  >
    <img
    ...
    {isPinned && (
      <div className="absolute top-3 left-3 bg-blue-600 text-white text-xs font-bold px-2 py-1 rounded-md shadow-lg flex items-center gap-1">
        <Pin size={10} /> PINNED
      </div>
    )}
  </div>
...
const SuggestedCreator = ({ user }) => (
  <div className="flex items-start gap-3 p-3 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer group">
    <Link to={`/author/${user._id}`} className="flex-shrink-0">
      {user.profileImage ? (
        <img
    ...
    <div className="flex-1 min-w-0">
      ...
      <button className="mt-2 text-xs font-bold text-blue-600 flex items-center gap-1 hover:underline">
...
const AuthorProfile = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (token) {
        const status = await request(`/api/user/${id}/follow-status`, 'GET', null, {
  ...
  if (error || !author)
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 text-center px-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Oops! Profile not found</h2>
          <button onClick={() => navigate('/home')} className="text-blue-600 font-bold underline">
            Go back home
          </button>
        </div>
      </div>
  ...
  const displayPosts =
    activeTab === 'posts' ? [...posts].sort((a, b) => (a._id === author.pinnedPostId ? -1 : 1)) : likedPosts;
              ...
              <div className="px-6 pb-6 relative">
                ...
                <div className="pt-4 flex justify-end gap-2">
                  {!isOwner ? (
                    ...
                    <button
                      ...
                      className="px-6 py-1.5 rounded-full border-2 border-gray-300 text-gray-600 font-bold text-sm hover:bg-gray-50"
                    >
                      Edit Profile
                    </button>
                  )}
                ...
                <div className="mt-8">
                  ...
                  <p className="text-gray-700 text-base mt-1 font-medium">
                    {author.bio || 'Crafting digital experiences @ Postify'}
                  </p>

                  <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-3 text-sm text-gray-500 font-medium">
                    <span className="flex items-center gap-1.5">
                      <Calendar size={14} /> Joined{' '}
                      {new Date(author.createdAt).toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}
                    </span>
                  </div>

                  <div className="mt-4 flex items-center gap-2 text-sm">
                    <button onClick={() => setModal('followers')} className="text-blue-600 font-bold hover:underline">
                      {author.followers?.length || 0} followers
                    </button>
                    <span className="text-gray-300">•</span>
                    <button onClick={() => setModal('following')} className="text-blue-600 font-bold hover:underline">
                      {author.following?.length || 0} following
                    </button>
                  </div>
                </div>
              </div>
                ...
                {suggestedPosts.length > 0 ? (
                  suggestedPosts.map((p) => (
                    ...
                    >
                      ...
                      />
                      <div className="flex-1 min-w-0">
                        <h4 className="text-sm font-bold text-gray-900 truncate leading-tight group-hover:text-blue-600 transition-colors uppercase">
                          {p.title}
                        </h4>
